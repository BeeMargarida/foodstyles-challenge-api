...
export const checkPassword = (password: string, hashedPassword: string) => {
  return new Promise<boolean>((resolve) =>
    compare(password, hashedPassword, (_err: any, same: boolean) => resolve(same))
  );
};

export const generateAccessToken = (data: Record<string, unknown>) => {
  return new Promise<string | undefined>((resolve, reject) => jwt.sign(data, JWT_SECRET, (err, token) => {
    if (err) reject(err);
    resolve(token);
  }));
};
